...
WHERE SeaID=@id;";
        ...
        public static void Delete(int id)
        {
            using (SqlConnection conn = DbHelper.GetConnection())
            {
                ...
                using (SqlCommand cmd1 = new SqlCommand("DELETE FROM SeasCountries WHERE SeaID=@id;", conn))
                {
                    cmd1.Parameters.AddWithValue("@id", id);
                    cmd1.ExecuteNonQuery();
                }

                using (SqlCommand cmd2 = new SqlCommand("DELETE FROM Seas WHERE SeaID=@id;", conn))
                {
        ...
        public static string GetNameById(int id)
        {
            using (SqlConnection conn = DbHelper.GetConnection())
            {
                const string sql = "SELECT Name FROM Seas WHERE SeaID=@id;";
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@id", id);
                    conn.Open();
